...
export const RecentActivity = ({ activities, topRoles }: Props) => {
  ...
  return (
    <div className="flex flex-col gap-4 h-full">
      <div className=" border border-primary bg-[#E5EAFF] rounded-lg lg:p-3 p-2 lg:py-5">
        <h1 className="lg:text-[22px] text-[18px] font-semibold text-primary">
          Recent Activity
        </h1>
        <div className="flex flex-col font-sora gap-3 mt-4">
          {activities.map((activity) => (
            <div
              key={activity.id}
              className="flex justify-between items-center lg:text-[18px]"
            >
              <p>
                {getActivityText(
                  activity.status,
      ...
      <div>
        ...
        <div className="flex flex-col gap-2 mt-4">
          {topRoles.map((role) => (
            <div
              key={role.role}
              className="flex justify-between items-center rounded-lg bg-[#0020A5] lg:px-3 lg:py-2 px-2 py-1 text-white font-semibold transition-all duration-500"
              style={{
